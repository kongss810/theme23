      ...
      .then((text) => {
        ...
        if (recommendations?.innerHTML.trim().length) {
          this.innerHTML = recommendations.innerHTML;
           var swiprelatedProductsSwiperer = new Swiper(`.related_productsAll .relatedProductsSwiper`, {
            observer: true,
            observeParents: true,
            breakpoints: {
              320: {
            ...
            navigation: {
              nextEl: `.related_productsAll .swiper-button-next`,
              prevEl: `.related_productsAll .swiper-button-prev`,
            },
          });
  ...
  class DesktopNavigation extends HTMLElement {
    ...
    openDropdown(parentElement) {
      ...
      let openingTimeout = setTimeout(() => {
        ...
        $(".header__wrapperBox").addClass("header__wrapperBoxs");
    ...
    closeDropdown(parentElement) {
      ...
      requestAnimationFrame(() => {
        ...
        $(".header__wrapperBox").removeClass("header__wrapperBoxs");
  ...
  class StoreHeader extends HTMLElement {
    connectedCallback() {
      ...
      if (this.isTransparent) {
        ...
        this.addEventListener("mouseenter", this._checkTransparentHeader.bind(this), true);
        this.addEventListener("mouseleave", this._checkTransparentHeader.bind(this));
...
// var stickyText = document.querySelector('.productStickyText_as'); // 新增
...
//       $(".header_all").css("transform", "translateY(-" + (announcementHeight+ headerHeight) +"px)");
//       // 同步隐藏 productStickyText_as
//       if (stickyText) {
//         stickyText.style.transform = "translateY(-" + ( headerHeight) + "px)";
//       }
...
//     //向上滚动 
//     if (!isHeaderVisible) { 
//       announcement.style.transform = 'translateY(0px)';  
//       header.style.transform = 'translateY(0px)'; 
//       // 同步还原 productStickyText_as
//       if (stickyText) { 
//         stickyText.style.transform = 'translateY(0px)';
//       }
...
function formatMoney(cents, format = "") {
  ...
  const placeholderRegex = /\{\{\s*(\w+)\s*\}\}/, formatString = format || window.themeVariables.moneyFormat;
...
class ProductItem extends HTMLElement {
    connectedCallback() {
      this.primaryImageList = Array.from(this.querySelectorAll(".card_mediaImg"));
      
      this.addEventListener("change", this._onColorSwatchChanged.bind(this));

    }
    _updatePrice(target) {
      const priceRegular = this.querySelector(".price-item--regular");
      const priceSale = this.querySelector(".price-item--sale");

      if (!priceSale || !target.dataset.price) return;

      const price = parseInt(target.dataset.price, 10);
      const compare = parseInt(target.dataset.comparePrice, 10);

      /* 有折扣 */
      if (compare && compare > price) {
        priceSale.textContent = formatMoney(price);      // ✅ 修正
        priceRegular.textContent = formatMoney(compare);  

        priceSale.removeAttribute("hidden");
        priceRegular.removeAttribute("hidden");
      }
      /* 无折扣 */
      else {
        priceSale.textContent = formatMoney(price);
        priceSale.removeAttribute("hidden");

        if (priceRegular) {
          priceRegular.setAttribute("hidden", "");
        }
      }
    }

    async _onColorSwatchChanged(event) {
      ...
      if (target.hasAttribute("data-variant-featured-media")) {
        ...
        this.primaryImageList.filter((image) => image.classList.contains("card_mediaImg") && image !== newImage).forEach((image) => image.setAttribute("hidden", ""));
      }
